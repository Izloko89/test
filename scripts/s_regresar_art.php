...
try{
	...
	foreach($_POST["items"] as $d){
		...
		if($cantidad==$d["cant"]){
			$entro=1;
			$sql="SELECT cantidad FROM almacen_inventario where id_articulo = ".$d["art"].";";
			$res=$bd->query($sql);
			$res=$res->fetchAll(PDO::FETCH_ASSOC);
			$cantidad=$res[0]["cantidad"];
			$suma=$cantidad+$d["cant"];
			$bd->query("UPDATE almacen_inventario set cantidad = ".$suma . " where id_articulo = ".$d["art"]);
		}else{
			$entro=0;
		}
