...
// This function added by P. Ziecina 2010
function GetViews($Database)
	{
		$xmlOutput = '';
		$query  = "SELECT VIEW_DEFINITION FROM INFORMATION_SCHEMA.VIEWS ";
		$result = mysql_query($query, $this->connectionId) or $this->HandleException();
...
// This function added by P. Ziecina 2010
	function GetProcedures($Database)
	{
		...
		if ($result)
		{
			...
			// Columns are referenced by index, so Schema and
			// Catalog must be specified even though they are not supported
			$xmlOutput .= '<FIELD><NAME>PROCEDURE_CATALOG</NAME></FIELD>';		// column 0
...
function myGetParametersOfProcedure($ProcedureName)
	{	
					$result = "";
...
if ($result)
		{		
			...
			$xmlOutput .= '<FIELD><NAME>PROCEDURE_SCHEMA</NAME></FIELD>';		// column 1
			$xmlOutput .= '<FIELD><NAME>PROCEDURE_NAME</NAME></FIELD>';		// column 2
			$xmlOutput .= '<FIELD><NAME>PROCEDURE_PARAMETERS</NAME></FIELD>';
			$xmlOutput .= '<FIELD><NAME>PROCEDURE_DIRECTION</NAME></FIELD>'; 
			$xmlOutput .= "</FIELDS><ROWS>";
			...
			while ($row = mysql_fetch_assoc($result)){
								...
								$xmlOutput .= '<ROW><VALUE/><VALUE/><VALUE>';
								$xmlOutput .= $row['Create Procedure'];
								$xmlOutput .= '</VALUE></ROW>';
			}
			$xmlOutput .= "</ROWS></RESULTSET>";
...
function GetParametersOfProcedure($ProcedureName)
	{
		$xmlOutput = "";
		
		//$query  = "SHOW CREATE PROCEDURE $ProcedureName ";
		$query  = "SELECT * FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_NAME='$ProcedureName' ";
					$result = mysql_query($query, $this->connectionId) or $this->HandleException();
		...
		if ($result)
		{
			...
			while ($row = mysql_fetch_array($result, MYSQL_ASSOC))
			{
				...
				// Separate type from size. Format is: type(size)
				/* if (preg_match("/(.*)\((.*)\)/", $row["ROUTINE_DEFINITION"], $ret))
				{
					$type = $ret[1];
					$size = $ret[2];
				}
				...
				{	*/
					...
					$datatype =  $row["ROUTINE_DEFINITION"];
					//$type =  $row["ROUTINE_DEFINITION"];
					$size = "";
					$direction = "";
				//	}	
				...
				$xmlOutput .= "<VALUE>" . $name . "</VALUE>";
				$xmlOutput .= "<VALUE>" . $datatype . "</VALUE>";
				//$xmlOutput .= "<VALUE>" . $size . "</VALUE>";
				$xmlOutput .= "<VALUE>".  $direction . "</VALUE></ROW>";
...
// This function changed by P. Ziecina 2010
	function SupportsProcedure()
	{	
		
		return '<SUPPORTSPROCEDURE status=true></SUPPORTSPROCEDURE>';
	}	
	/*
