...
<form id="f_clientes_fiscal" class="formularios" style="margin-left:30px;">
  <h3 class="titulo_form">DOCUMENTACIÓN</h3>
  <input type="hidden" name="id" class="id" />
  <input type="hidden" name="id_empresa" value="<?php echo $_SESSION["id_empresa"]; ?>" />
    <div class="campo_form">
        <label class="label_width">IFE/INE/Clave</label>
        <input type="text" name="rfc" class=" mayuscula rfc">
    </div>
    ...
    <div class="campo_form">
        <label class="label_width">No. Emisión</label>
        <input type="text" name="nombrecomercial" class="nombrecomercial">
    </div>
    <div class="campo_form">
        <label class="label_width">No. Vertical</label>
        <input type="text" name="direccion" class="direccion direccion_fiscal">
    </div>
    <div class="campo_form">
        <label class="label_width">Comprobante Domiciliario</label>
        <input type="text" name="colonia" class="colonia colonia_fiscal">
    </div>
    <div class="campo_form">
        <label class="label_width">Folio Fiscal</label>
        <input type="text" name="ciudad" class=" ciudad ciudad_fiscal">
    </div>
    <div class="campo_form">
        <label class="label_width">RFC Emisor</label>
        <input type="text" name="estado" class="estado estado_fiscal">
    </div>
    <div class="campo_form">
        <label class="label_width">RFC Destinatario</label>
        <input type="text" name="cp" class="cp cp_fiscal">
    </div>
    </form>
	</td>
	</tr>
...
<div id="destino" align="center"></div>
		...
		<div class="formularios">
	<h3 class="titulo_form">Imagenes</h3>
	<div class="mostrar" id="Imagenes"></div>

<!--<input type="text" name="aidi" id="aidi" onchange="realizaProceso($('#aidi').val());return false;"></p>-->
<br>
<br>
<center>
		<input type="file" id="file" name="file[]" accept="image/*" multiple="multiple"/><img id="cargando"/>
		<br>
		<input type="button" id="asd" value="Guardar"/>
		</center>
		<script>
			$(document).ready( function (e){
				...
				var data = new FormData();
					$.each($('#file')[0].files, function(i, file) {
						...
						data.append('cliente', $('#id_cliente').val());
					});
						$.ajax({
							url: "scripts/guardaImagenes.php",
							data: data,	
							cache: false,
							contentType: false,
							processData: false,
...
<h3 class="titulo_form">Listado de clientes registrados</h3>
	...
	if(count($clientes)>0){foreach($clientes as $art=>$d){
		...
		echo '<td>'.$d["nombre"].'</td>';
	echo	'<td class="eliminar_tevento" onclick="eliminar_art(' . $cont . ',' . $d["clave"] . ')"></td>';
		echo '</tr>';
		$cont++;
	}//foreach
	}//if end ?>
