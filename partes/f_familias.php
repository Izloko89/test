...
<?php
	try{
		...
		$res=$bd->query("SELECT * FROM familias order by nombre;");
		$cont = 1;
		foreach($res->fetchAll(PDO::FETCH_ASSOC) as $v){
			echo '<tr class="familia fondo_azul"><td ><div align="left" >'.$v["nombre"].'</div></td><td colspan="2" align="right"><span class="eliminar_tevento" onclick="eliminar_art(' . $cont . ', ' . $v["id_familia"] . ');"> <input id="idTipo" type="hidden" value="' . $v["id_familia"] . '"/></td></tr>';
			$cont++;
		}
		...
		$.ajax({
			...
			success: function(r){
			  if(r){
				...
				alerta("info","<strong>Familia</strong> Eliminada");
				$(".volver").click();
			  }else{
				alerta("error", r);
			  }
